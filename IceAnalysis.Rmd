...
#weighted kernel
ggplot(mxy_time,aes(x=MeanIce))  + geom_density(fill="red") + geom_density(aes(weight=LastTime/sum(LastTime)),fill="black",alpha=0.3)
```
...
ggplot(data=mxy,aes(x=dist_ice)) + geom_histogram() + labs(x="Distance to ice edge") + theme_bw() 
...
ice_frame$Month<-factor(ice_frame$Months,levels=month.name)
ggplot(ice_frame,aes(x=as.numeric(daydiff),y=icediff)) + geom_point() + labs(y="Change in ice concentration",x="Days since observation")

ggplot(ice_frame,aes(x=as.numeric(daydiff),y=icediff)) + geom_smooth() + labs(y="Change in ice concentration",x="Days since observation") + facet_wrap(~Month)
```
